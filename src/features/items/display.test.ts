...
describe('subtitleOf', () => {
  ...
  it('author の @ が二重にならない', () => {
    expect(
      subtitleOf(t, { siteName: null, author: '@foo', url: 'https://x.com/a', savedAt: NOW }, NOW),
    ).toBe('@foo · 今日');
  });
  ...
  it('author が無ければ siteName を出す', () => {
    expect(
      subtitleOf(
        t,
        { siteName: 'Zenn', author: null, url: 'https://zenn.dev/a', savedAt: NOW },
        NOW,
      ),
    ).toBe('Zenn · 今日');
  });
  ...
  it('どちらも無ければホスト名を出す', () => {
    ...
    expect(
      subtitleOf(t, { siteName: null, author: null, url: 'https://qiita.com/a', savedAt }, NOW),
    ).toBe('qiita.com · 12日前');
  });
});
...
describe('relativeDays', () => {
  it.each([
    ...
    expect(relativeDays(t, days)).toBe(expected);
  });

  // 煽らない
  it('感嘆符や絵文字を含まない', () => {
    for (const days of [0, 1, 30, 100]) {
      expect(relativeDays(t, days)).not.toMatch(/[!！🎉🔥]/u);
    }
  });
});
...
describe('unreadHeadline', () => {
  it('挨拶ではなく状態を出す', () => {
    expect(unreadHeadline(t, 23)).toBe('未読 23 件');
    expect(unreadHeadline(t, 0)).toBe('未読 0 件');
  });

  it('挨拶語を含まない', () => {
    expect(unreadHeadline(t, 5)).not.toMatch(/おはよう|こんにちは|Good morning/i);
  });
});
